...
export let middleware = (request) => {
    ...
    // if(request?.nextUrl?.hostname?.includes('unaltru') || request?.nextUrl?.hostname?.includes('localhost')) {
    //     console.log({ req: request?.nextUrl })
    //     return NextResponse.redirect(new URL('https://www.notion.so/angeloreale/UNA-United-Natural-Altruists-6246fddb819d4bf28240a6016c613f45?pvs=4'));
    // }
