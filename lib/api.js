...
async function fetchGraphQL(query, preview = false) {
  return fetch(
    `https://graphql.contentful.com/content/v1/spaces/${process.env.CONTENTFUL_SPACE_ID}/environments/${process.env.CONTENTFUL_ENVIRONMENT || "migration-0001"}`,
    {
      method: 'POST',
      headers: {
...
function extractEvent(fetchResponse, { locale }) {
  const event = fetchResponse?.data?.eventsCollection?.items?.[0]
  const superData = event.superData[locale]
  const decEvent = { ...event, ...superData }
  console.log({ decEvent, superData: JSON.stringify(superData) })
  return decEvent
}

function extractEvents(fetchResponse) {
  const items = fetchResponse?.data?.eventsCollection?.items
  return items
}
...
export async function getEvent(slug, preview, locale = 'en') {
  ...
  return {
    event: extractEvent(entry, { locale })
  }
}
