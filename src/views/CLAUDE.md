...
| `DoView` | `do/doView.tsx` | Task management hub (orchestrates forms + ListView) | `/api/v1/tasks`, `/api/v1/jobs`, `/api/v1/profiles/by-ids` (via ListView child) |
...
`src/views/index.ts` exports the public view components (`BeView`, `ChatView`, `DashboardView`, `DoView`, `InvestView`, `ListView`, `MoodView`, `ProfileView`, `SettingsView`).
