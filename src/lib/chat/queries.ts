...
async function getReplyStats(rootIds: string[]) {
  ...
  await Promise.all(rootIds.map(async (rootId) => {
    const [replyCount, latestReply] = await Promise.all([
      prisma.chatMessage.count({ where: { threadRootMessageId: rootId, deletedAt: null } }),
      prisma.chatMessage.findFirst({
        where: { threadRootMessageId: rootId, deletedAt: null },
...
async function getUnreadCountForRoom(userId: string, room: { channelId?: string; dmConversationId?: string; lastMessageAt?: Date | null }) {
...
export async function getUnreadCount(userId: string) {
  ...
  return roomCounts.reduce((total, count) => total + count, 0)
}
...
export async function getChatSidebar(userId: string) {
  ...
  const [orgs, channels, dms] = await Promise.all([
