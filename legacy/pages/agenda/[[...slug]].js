...
export default function Page({ page, agenda, generatedIn, cal }) {
  ...
  const handleClick = () => {
    ...
    window.open('https://www.dupip.com/chat')
  }
  const handleClose = () => {
    setCtaOpen(false)
  }
  ...
  const url = `https://www.dupip.com${orig !== 'default' ? `/${locale}` : ''}/${actualSlug}`
  ...
  return (
    <>
      <Head>
        ...
        <meta
          property="og:image"
          content={image || "https://www.dupip.com/og-image.png" + "?fm=jpg&w=512"}
        />
        <meta
        ...
        <link rel="alternate" hrefLang="x-default" href={`https://www.dupip.com/${actualSlug}`} />
        {Object.keys(localeMap).map((locale) => {
          return <link key={locale} rel="alternate" hrefLang={locale} href={`https://www.dupip.com/${locale}/${actualSlug}`} />
        })}
      </Head>
