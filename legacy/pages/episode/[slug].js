...
export default function Episode({ episode, preview }) {
  ...
  const title = `${episode?.title} — Dupip`

  const { locale: orig, pathname } = useRouter()
  const locale = orig === "default" ? "en" : orig

  const url = `https://www.dupip.com/${orig !== 'default' ? `${locale}/` : ''}episode/${episode?.url}`
  ...
  return (
    <>
      <Head>
        ...
        <meta
          property="og:image"
          content={`${(episode?.placeholder?.url || episode?.image?.url || "https:`//www.dupip.com/og-image.png") + "?fm=jpg&w=512"}`}
        />
        <meta property="twitter:image" content={`${(episode?.placeholder?.url || episode?.image?.url || "https://www.dupip.com/og-image.png") + "?fm=jpg&w=512"}`} />
        <meta
        ...
        <link rel="alternate" hrefLang="x-default" href={`https://www.dupip.com/episode/${episode?.url}`} />
        {Object.keys(localeMap).map((locale) => {
          return <link key={locale} rel="alternate" hrefLang={locale} href={`https://www.dupip.com/${locale}/episode/${episode?.url}`} />
        })}
      </Head>
